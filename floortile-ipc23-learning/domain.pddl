...
(:legality-axiom (illegal) (exists (?r - robot)
  (not (exists (?t - tile) (robot-at ?r ?t)))))
(:legality-axiom (illegal) (exists (?r - robot ?t1 ?t2 - tile)
  (and (robot-at ?r ?t1) (robot-at ?r ?t2) (not (= ?t1 ?t2)))))

;; a tile is clear iff it is not painted and there is no robot on
(:legality-axiom (illegal) (exists (?t - tile ?r - robot ?c - color)
...
(:legality-axiom (illegal) (forall (?t - tile)
  (not (exists (?r - robot) (robot-at ?r ?t)))))

;; there are exactly two colors
(:legality-axiom (illegal) (forall (?x ?y - color) (= ?x ?y)))
(:legality-axiom (illegal) (exists (?x ?y ?z - color)
  (and (not (= ?x ?y)) (not (= ?x ?z)) (not (= ?y ?z)))))

;; each robot has exactly one color
(:legality-axiom (illegal) (exists (?r - robot) (forall (?c - color)
  (not (robot-has ?r ?c)))))

;; all colors are available
(:legality-axiom (illegal) (exists (?c - color) (not (available-color ?c))))

;; the direction predicates form inverse pairs
(:legality-axiom (illegal) (exists (?x ?y - tile) (and (up ?x ?y) (not (down ?y ?x)))))
(:legality-axiom (illegal) (exists (?x ?y - tile) (and (down ?x ?y) (not (up ?y ?x)))))
(:legality-axiom (illegal) (exists (?x ?y - tile) (and (left ?x ?y) (not (right ?y ?x)))))
(:legality-axiom (illegal) (exists (?x ?y - tile) (and (right ?x ?y) (not (left ?y ?x)))))

;; the direction predicates are irreflexive
(:legality-axiom (illegal) (exists (?x - tile) (up ?x ?x)))
(:legality-axiom (illegal) (exists (?x - tile) (down ?x ?x)))
(:legality-axiom (illegal) (exists (?x - tile) (left ?x ?x)))
(:legality-axiom (illegal) (exists (?x - tile) (right ?x ?x)))

;; there is at most one tile in each direction
(:legality-axiom (illegal) (exists (?x ?y ?z - tile)
...
(:legality-axiom (tl ?x - tile) (not (exists (?y - tile) (or (up ?y ?x) (left ?y ?x)))))
(:legality-axiom (illegal) (not (exists (?x - tile) (tl ?x))))
(:legality-axiom (illegal) (exists (?x ?y - tile) (and (tl ?x) (tl ?y) (not (= ?x ?y)))))

;; columns of the grid
(:legality-axiom (col ?x - tile) (tl ?x))
(:legality-axiom (col ?x - tile) (exists (?y - tile) (and (col ?y) (right ?x ?y))))
(:legality-axiom (colOf ?x ?c - tile) (and (col ?c) (= ?x ?c)))
(:legality-axiom (colOf ?x ?c - tile) (exists (?y - tile)
  (and (colOf ?y ?c) (down ?x ?y))))

;; rows of the grid
(:legality-axiom (row ?x - tile) (tl ?x))
(:legality-axiom (row ?x - tile) (exists (?y - tile) (and (row ?y) (down ?x ?y))))
(:legality-axiom (rowOf ?x ?r - tile) (and (row ?r) (= ?x ?r)))
(:legality-axiom (rowOf ?x ?r - tile) (exists (?y - tile)
...
(:legality-axiom (illegal) (exists (?x ?c1 ?c2 - tile)
  (and (not (= ?c1 ?c2)) (colOf ?x ?c1) (colOf ?x ?c2))))
(:legality-axiom (illegal) (exists (?x ?r1 ?r2 - tile)
...
(:legality-axiom (illegal) (exists (?x ?y - tile ?c - color)
